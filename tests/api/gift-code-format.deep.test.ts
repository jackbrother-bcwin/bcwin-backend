/**
 * Brand gift codes: BCWIN0X + 8, no hyphen.
 * Redeem trims + uppercases; old YYYYMMDD-… codes still match.
 */
...
describe("Gift code format", () => {
    test("new codes are BCWIN0X + 8 A–Z/0–9 with no hyphen", () => {
        const codes = new Set<string>();
        for (let i = 0; i < 40; i++) {
            const code = generateGiftCode();
            expect(isBrandGiftCode(code)).toBe(true);
            expect(code.includes("-")).toBe(false);
            expect(code.startsWith("BCWIN0X")).toBe(true);
