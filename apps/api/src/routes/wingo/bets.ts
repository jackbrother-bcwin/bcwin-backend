...
const logger = new Logger("wingo-bets");
    ...
    app.openapi(placeBetRoute, async (c) => {
        try {
            ...
            const { result, updatedUser, penaltyChanged } = await prisma.$transaction(
                async (tx) => {
                    // Atomic: balance >= betAmount (blocks concurrent overdraft)
                    ...
                    const result = await tx.wingoBet.create({
                        data: {
                            userId: user.id,
                            periodId,
                            betAmount,
                    ...
                    const penaltyChanged = await detectPlacedIllegalBet(tx, "WINGO", result);
            ...
            if (!user.isDemo) {
                WebSocketManager.publishToTopic("admin-wingo-bets", {
                    betId: result.id,
                    userId: user.id,
            ...
            // XP was incremented with the bet — unlock VIP levels immediately
            ...
            return c.json(
                {
                    ...
                    bet: {
                        ...
                        result: result.wingoBetResult
                            ? {
                                  isWin: result.wingoBetResult.isWin,
                                  winAmount: result.wingoBetResult.winAmount,
                                  multiplier: result.wingoBetResult.multiplier,
                              }
                            : null,
    ...
    app.openapi(getUserBetsRoute, async (c) => {
        try {
            ...
            const [bets, total] = await Promise.all([
                ...
                prisma.wingoBet.count({ where: whereClause }),
            ]);
            ...
            const result = {
                bets: bets.map((bet) => ({
                    ...
                    status: bet.status,
                    result: bet.wingoBetResult
                        ? {
                              isWin: bet.wingoBetResult.isWin,
                              winAmount: bet.wingoBetResult.winAmount,
                              multiplier: bet.wingoBetResult.multiplier,
                          }
                        : null,
            ...
            // Short TTL only: 1h cache + place→reload race can hide later bets
            // on the same period (stale list overwrites after invalidate).
